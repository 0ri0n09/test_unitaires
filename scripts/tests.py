...
def run_tests():
    ...
    check(is_valid_practice(group) == True, "Test 2 (begin at 1)")
    ...
    check(is_valid_practice(group) == False, "Test 2 (stay at 1)")
    ...
    df = data.copy()
    df = calculate_streaks_and_lives(df)
    check(df['Streak'].iloc[0] == 1, "Test 5 (begin at 1)", f"Expected Streak: 1, Found: {df['Streak'].iloc[0]}")
    check(df['Streak'].iloc[-1] == 1, "Test 5 (stay at 1)", f"Expected Streak: 1, Found: {df['Streak'].iloc[-1]}")

    # Test 6 : Vérifier que plusieurs pratiques valides le même jour sont comptées comme une seule pratique
    df = data.copy()
    new_row = data.iloc[0].copy()
    ...
    df = pd.concat([df, pd.DataFrame([new_row])], ignore_index=True)
    df = calculate_streaks_and_lives(df)
    check(df['Streak'].iloc[0] == 1, "Test 6", f"Expected Streak: 1, Found: {df['Streak'].iloc[0]}")

    # Test 7 : Vérifier les vies après des jours manqués
    df = data.copy()
    df.loc[df.index[-1], 'Date'] = df.loc[df.index[0], 'Date'] + timedelta(days=10)
    df = calculate_streaks_and_lives(df)
    check(df['Lives'].iloc[-1] == 2, "Test 7", f"Expected Lives: 2, Found: {df['Lives'].iloc[-1]}")

    # Test 8 : Vérifier les streaks après des jours manqués
    df = data.copy()
    df.loc[df.index[-1], 'Date'] = df.loc[df.index[0], 'Date'] + timedelta(days=10)
    df = calculate_streaks_and_lives(df)
    check(df['Streak'].iloc[-1] == 1, "Test 8", f"Expected Streak: 1, Found: {df['Streak'].iloc[-1]}")

    # Test 9 : Vérifier l'incrémentation des streaks avec des pratiques valides
    df = data.copy()
    for _ in range(2):
        new_row = data.iloc[0].copy()
        new_row['Date'] = new_row['Date'] + timedelta(days=1)
        df = pd.concat([df, pd.DataFrame([new_row])], ignore_index=True)
    df = calculate_streaks_and_lives(df)
    check(df['Streak'].iloc[-1] == 2, "Test 9", f"Expected Streak: 2, Found: {df['Streak'].iloc[-1]}")

    # Test 10 : Vérifier la pratique valide avec des niveaux mixtes
    group = pd.DataFrame({
        'Niveau': [1, 2],
        'Allonge': [True, True],
        'Assis': [True, True]
    })
    check(is_valid_practice(group) == True, "Test 10")

    # Test 11 : Vérifier la pratique invalide avec un nombre insuffisant d'exercices de niveau 1
    group = pd.DataFrame({
        'Niveau': [1],
        'Allonge': [True],
        'Assis': [True]
    })
    check(is_valid_practice(group) == False, "Test 11")

    # Test 12 : Vérifier la réinitialisation des vies après la perte de toutes les vies
    df = data.copy()
    for _ in range(3):
        new_row = data.iloc[0].copy()
        ...
        df = pd.concat([df, pd.DataFrame([new_row])], ignore_index=True)
    df = calculate_streaks_and_lives(df)
    check(df['Lives'].iloc[-1] == 2, "Test 12", f"Expected Lives: 2, Found: {df['Lives'].iloc[-1]}")

    # Test 13 : Vérifier la regénération de vie et de reset après une fin de série 
    df = pd.DataFrame({
        'Date': [1618937885, 1619024285, 1619110685, 1619197085, 1619283485],
        'Niveau': [2, 2, 2, 2, 2],
        'Allonge': [True, True, True, True, True],
        'Assis': [True, True, True, True, True],
        'SessionID': [
            'ed73e2a7-8f8a-493c-9388-c7cc4714b0ad',
            'ed73e2a7-8f8a-493c-9388-c7cc4714b0ad',
            'ed73e2a7-8f8a-493c-9388-c7cc4714b0ad',
        ...
        'formattedDate': ['20/04/2021', '21/04/2021', '22/04/2021', '23/04/2021', '24/04/2021'],
        'Streak': [0]*5,
        'Lives': [2]*5
    })
    df['Date'] = pd.to_datetime(df['Date'], unit='s')
    df = calculate_streaks_and_lives(df)
    check(df['Lives'].iloc[-1] == 2, "Test 13 (life)", f"Expected Lives: 2, Found: {df['Lives'].iloc[-1]}")
    check(df['Streak'].iloc[-1] == 5, "Test 13 (streak)", f"Expected Streak: 5, Found: {df['Streak'].iloc[-1]}")
    
    # Test 14 : Vérifier que les vies ne dépassent pas le maximum après une longue série de pratiques valides
    df = data.copy()
    for _ in range(30):
        new_row = data.iloc[0].copy()
        new_row['Date'] = new_row['Date'] + timedelta(days=1)
        df = pd.concat([df, pd.DataFrame([new_row])], ignore_index=True)
        df = calculate_streaks_and_lives(df)
    check(df['Lives'].iloc[-1] == 2, "Test 14", f"Expected Lives: 2, Found: {df['Lives'].iloc[-1]}")
